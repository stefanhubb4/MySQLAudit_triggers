...
DROP PROCEDURE IF EXISTS audit.proc_create_after_insert_trigger //

CREATE PROCEDURE audit.proc_create_after_insert_trigger(IN full_original_table_name VARCHAR(255), IN full_audit_table_name VARCHAR(255))
BEGIN
    ...
    SET stmt = CONCAT('DELIMITER //\n',
                      'CREATE TRIGGER ', @original_db_name, '.after_insert_', @original_table_name,
                      ' AFTER INSERT ON ', @original_db_name, '.', @original_table_name,
                      ' FOR EACH ROW\n',
                      'BEGIN\n',
                      ...
                      '    VALUES (', select_new, ', NULL, NOW(), USER(), @@hostname, ''INSERT'');\n',
                      'END//\n',
                      'DELIMITER ;');
